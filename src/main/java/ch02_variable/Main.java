...
/*
    주석(comment) : Java 프로그램ㄹ의 경우 폴더 및 파일의 전체 코드를 컴파일링 하는데,
...
public class Main {
    public static void main(String[] args) {
        System.out.println("안녕하세요, 제 이름은 김재현입니다.");
//        System.out.println(안녕하세요, 제 이름은 김재현입니다.); // 오류 발생
        System.out.println(1);
        System.out.println('1');
        System.out.println("1");
        /*
            이상과 같은 방식의 코드 ㅐ대입을(매번 데이터를 집적 사용하는 방식)
            하드 코딩이라는 표현을 씁니다.
            하드 코딩 : 명령문에 데이터(변수에 대입되지 않은 데이터인데 이를 리터럴(Literal))를 바로 집어넣는 것으로, '지양'되는 방식.

            그렇다면 권장되는 방식은 ? 데이터를 '변수'에 대이바고, 변수를 통해서 명령을 내리는 방식
        ...
        System.out.println('안' + '근' + '수'); // 결과갑 : 144732

        System.out.println("" + name1 + name2 + name3); // 결과값 : 안근수

        /*
            이상의 코드에서 알 수 있는 점은 컴퓨터가 생각보다 명청하다.
            개발자인 저희가 하나하나 지정을 해줘야 한다는 점입니다.
            '문자'와 '문자열'은 서로 다른 개념인데,
            문자(하나짜리)를 세 번 더했을 경우에는 더이상 문자가 아니라 문자열루 인식되기
            때문에 원하는 방식으로 출력되지 않습니다. 144 어쩌고로 출력됐죠.

            그렇기 때문에 74번 라인에서 sout()dml () 내부에 ""를 넣얼주니까,
            원하는 결과값을 얻을 수 있었습니다.
        ...
        String name = "안근수";
        String major = "영어교육과";
        String currentJod = "코리아 it 아카데미 국비 강사";
        String previousJob = "영어 교사";

        System.out.println(name);
        System.out.println(major);
        System.out.println(currentJod);
        System.out.println(previousJob);

        System.out.println("안녕하세요, 제 이름은" + name + "입니다. 제 전공은 " + major
            + "이고, 그래서 제 예전 직업은 " + previousJob + "였습니다. 현재는" + currentJod + " 을 가지고 있습니다.");
        ...
        /*
            제가 위에 선언한 변수들에 여러분들에게 적합한 ㄹ데이터로 ' 초기화' 하여
            '저는 코리아 it 아카데미 7월 국비과정을 수강하고 있는 누구누구입니다. ?? 살입니다. -> 줄바꾼
            제 전공은 어쩌고저쩌고 학과이며, nbti는 블라블라입니다. 열심히할게요!!!'
        ...
        System.out.println(" 제 전공은 " + major10 + " 학과이며," + nbti10 + " 입니다. 열심히할게요!!!");

        //sout 한 번 써서 개형하는 방법
        System.out.println("저는 코리아 it 아카데미 7월 국비과정을 수강하고 있는 " + name10 + "입니다." + age10 + " 살입니다.\n " +
                "제 전공은 " + major10 + " 학과이며," + nbti10 + " 입니다. 열심히할게요!!!");

        // enter 키 위에 원화 표시 있습니다. 원화표시가 한국에서만 먹히는 형태고 원래는 역슬러시만 표기되어있습니다.
        // \n -> 개형이 일어납니다.
        // \t -> 탭 친 효과가 일어납니다.

        // ch03_operator 패키지 생성 -> ㅒ
    }
}
