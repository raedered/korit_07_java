...
public class CentralControl {
    ...
    public void addDevice(Power device) {
        ...
        /*
            ...
            현재 보시면 method의 결과값을 가지고 다시 . 찍어서 그다음 method를 호출했습니다.
            이상의 개념을 chaining method라고 해서 return값의 유형을 명확하게 알고 있어야
            그다음 어떤 method를 실행시킬 것인지를 알 수 있습니다.
         */
    }
    ...
    public void powerOn() {
        ...
        for(int i = 0; i < deviceArray.length; i++) {
            if(deviceArray[i] == null){
                ...
                // 현재 반복만 종료하고 그다음 반복문으로 넘어갑니다.
                // 즉 deviceArray[3] 주소지에 객체가 없어 null이 잇다면
                // 67번 라인의 조건문이 실행될텐데
    ...
    /*
        해당 배열 내에 각 객체들이 들어가있습니다.
        객체명.getClass().getSinpleName() 을 활용하면 클래스 네임만 출력된다는것을 확인 할 수 있는데,
        deviceArray를 반복 돌려서 몇 번 인뎃스에 어떤 클래스의 객체가 있는지 표시하는
        showInfo 메서드를 정의하시오.
        실행 예
        슬롯 [ 1 ] 번 : Conputer
        슬롯 [ 2 ] 번 : LED
    ...
    public void performSecificMethod() {
        for(Power device : deviceArray) {
            if(device instanceof AirConditioner) {  // Power의 서브클래스가 절못된 다운캐스팅을 하지 않도록 하는 조건문
                ...
                printer.print();
            } else {        // 아직 instanceof 연산자를 통해서 조건물을 추가하지 못한 class및
                            // 추후 update를 통해서 다른 기기들을 지원할 때는 } else 사이에 else if문을 추가해야함
                System.out.println("실행할수 없습니다.");
            }
        }
    }

    // 이상의 수업 과정들을 전부 거쳣 을때, Printer 클래스를 정의하고 Power interface를 implement하고
    // 고유 메서드로 print() -> "프린터가 인쇄를 합니다" 출력도리 수 있도록 정의하고,
    // Main에서 printer1을 cetralControl에 추가 on() / off()를 실행한 후게
    // print() 메서드도 centralControl을 통헤 실행하시오.
