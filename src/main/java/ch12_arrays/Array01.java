...
//        System.out.println(i);          // 결과값 : 1
        ...
        // 즉 element들을 확인하기 위해서는 배열 배부로 직적 탐색하는 과정이 필요합니다.
...
//        System.out.println(arr01[4]);
        ...
        // 어, index만 빼고 나머지가 반복되네? 반복문 써봐야겟네? 하면 개발자 지망생
        for(int i = 0; i < 5; i++){
            System.out.print(arr01[i] + " ");
        }
        System.out.println();

        for (int i = 0; i < arr01.length ;  i++){ // arr01.lenght 형태로 배열 내부의 데이터 개수가 바귀더라도 문제 없음
            System.out.print(arr01[i] + " / ");
        }

        // 근데 우리가 필드를 참조할 때 객체명.필드 형태로 참조했엇습니다.
        // bank1.accountHolder와 같은 방식으로 참도 가능했습니다.
        // 그리고 field를 직접 참조했다는 말은 length 필드는 public이라고 볼 수 있겠네요

        // 그렇다면 배열은 객체인가    -> 환전히 그렇다고는 x / 하지만 field로 length를 가진다
        // 빈 배열 생성

        int[] arr02 = new int[10]; // 열개짜리 방을 가진 비어있는 배열을 생성했습니다.
        ...
        // arr01 내부에 잇는 1, 2, 3, 4, 5는 굳이 따지자면 arr01[0], arr01[1], ..., arr01[4]라는 변수명을 지니고
        // 있다고 볼 수 있습니다.
        ...
        // arr02dp 현재 21, 222, 23, ...., 30이 대입되어 있습니다.
        // 42, 44, 46, ... 60이 될 수 잇도록 반복문을 작성하고
        // 42, 44, 46, ... 60, 으로 추력하시오.
