...
/*
    ...
    일반 for 문과의 차이점 :
        일반 for문의 경우 index 넘버(주소지)를 명확하게 알고 잇어야 하지만
        일일이 element의 숫자를 세는 것이 번거롭길 때문에
        arr01.lenght와 같은 방식으로 일종의 field 참조를 통해 int 값을 추출하여
        대입해야만 했습니다.

        그런데 잘 생각해보면 for (int i = 0; ...) 부분에서 int i는
        for문 내부에서 선언되고, 내부에서만 사용이 가능합니다.

        이를 응용하여 배열 내의 element에 for문 내부엣만 사용하는 변수에 대입하는
    ...
    형식 :
    for(자료형 변수명 : 반복가는객체(배열)) {
        반복실행문
    }
 */
public class Array10EnhanceFor {
...
//        }
        ...
        for (int number : nums) {   // int[]
            ...
            // 대입이 된 상태로 39번 라인을 실행시킵니다 "1"까지 출력 된니다.
            // 그다음 반복으로 넘어가서 nums의 두 번재 element인 2를 number에 대입
            // 39번 실행
            // 다음 3 대입
            // 39 실행
            // ... 200 대입
            // 39번 라인 실행하고, 마무리(마무리 시점은 마지막 element 대입후
            // 반복 실행문 실행 후)
            // 이후에 반복문 바깥에서는 int i 와 동일하게
            // number 변수를 사용할수 없습니다.
            // number = 201; 오류방생
