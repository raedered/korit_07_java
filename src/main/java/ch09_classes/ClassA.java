...
/*
    1. 클래스란 ?
        ...
        해동(가속, 정지 등)을 코드로 표현할 수 있슴.

        2. 객체(Object)란 ?
            클래스를 설계도로 가정햇을 대, 실제 사용할 수 있는 자동차(인스턴스)는 객체라고 함.
            ...
            예를 들어,
                1. "사람"이라는 클래스를 고려할 때,
                2. "홍길동", "김철수"는 이 클래스에서 생성된 객체임, 근대 "아반떼"도 객체긴 함.
                3. "홍길동"은 "사람"이라는 특정 클래스의 '인스턴스'에 해당.
 */
public class ClassA {
    // 필드(속성) / 인스턴스 변수 / 맴버 변수 정의
    // 밑에 뭔가 변수들을 선언만 했다가는 것을 확인할 수 있습니다.
    int num;
    String name;
    double score;
