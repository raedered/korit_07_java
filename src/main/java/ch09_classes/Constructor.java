...
/*
    ...
    특징:
        ...
        생성자의 정의 방식
            1. 기본 생성자 : 매개변수가 없는 생상자
                (사실 클래스를 만들 대 default로 생성되는 형태로 정의 하지 않아도 될 때가 있음)
                그래서 ClassA, Car에서 기본생성자를 정의하지 않고
                ClassAMain, CarMain에서 객체를 생성핼봤었습니다. -> 복습필요
                    정확하게는 default로 생성된 기본 생성자를 Main에서 호출해봤다고 볼 수 dlTtmqslek.
                    호출 결과 -> 객체 생성
...
public class Constructor {
    ...
    // 기본 생성자 정의 -> 원래 기본정으로 만들어집니다.
    // 그리고 매개변수의 유무만 있어서 call() / call2() 유형이라고 했으니까
    ...
    // 때문에 기본 생성자와 매개변수 생성자를 둘다 사용하고 싶다면
