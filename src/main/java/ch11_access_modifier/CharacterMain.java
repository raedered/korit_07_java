...
/*
    ...
    당야한 접근 수준의 필드와 메서드를 갖는 캐릭터 클래스를 정의하고
    ...
    2. 메서드 정의
    getHealth() -> 채력 값을 반환하는 getter -> public / 이미 자동완성으로 만들었습니다.
    attack() -> rhdrurtl zhsthf ckddp
        name 이 power로 곡격
        name 이(rk) 경험치 amount을(를) 얻었습니다. -> 라고 두줄 출력되게 작성할 것. call1() 유형
    heal() -> 채력을 10 회복하고, 현재 채력 출력
        채력이 10 회복되었습니다. 현재 채력 : health(숫자로 출력돼야 합니다.
    gainExp(int amount) -> call2() 유형이고 경험치를 증가시키는 메서드(내부전용)
        실행 예
        name이(가) am
 */
class Character {
    ...
    public Character(String name, String skill, int power) {
        ...
        this.health = 100;      // 초기 채력(매개변수는 없죠. 객체 생성시에 무조건 체력 100으로 고정된다는 의미)
        this.exp = 0;
    }
    ...
    public void heal() {
        health += 10;
        System.out.println(name + "의 체력이 회복되었습니다. 현재 채력 : " + health);
    }

    public void warrior () {
        System.out.println("이름" + name);
        System.out.println("채력" + health);
...
/*
    이상의 코드에서 여러분들일ㄹ 중점적으로 학습해야 할 점은
    접근 지정자의 설정 수준에 따라서 setter / getter가 필요한 사례가 있고 아닌 사례가 있다는 점입니다.
    예를 들어 게임을 하거나 SNS를 한다고 쳣을 때 닉네임의 경우는 누구나 볼 수 있죠.
    그렇다면 닉네임 필드는 public으로 설정되어잇을 겁니다.
    그렇기 때문에 getter를 통한 닉네임의 호출은 필요가 없어서 이상의 코드에서 getName() 메서드는 쓸모가
    없을 겁니다.

    우리는 어떤 클래스/필드/메서드를 어떤 접근지정자를 설정해야할지도 고민해야겠습니다.

    하지만 현제 수업 상황에서는 웬만하면 필드는 private으로 설정, getter/ setter를 통한 우회 설정 및 조회를
    목표로 하겠습니다.
 */